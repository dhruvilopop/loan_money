...
export default function AgentDashboard() {
  ...
  const openSessionDialog = (loan: Loan) => {
  ...
  const sessionCreated = loans.filter(l => l.status === 'SESSION_CREATED');
  ...
  const stats = [
    ...
    { label: 'Awaiting Session', value: formCompleted.length, icon: ClipboardCheck, color: 'text-violet-600', bg: 'bg-violet-50' },
      ...
      case 'session':
        return (
          <Card className="bg-white shadow-sm border-0">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ClipboardCheck className="h-5 w-5 text-violet-600" />Create Session
              </CardTitle>
              <CardDescription>Applications with completed verification. Create loan session for customer approval.</CardDescription>
            </CardHeader>
            <CardContent>
              {formCompleted.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  <ClipboardCheck className="h-12 w-12 mx-auto mb-3 text-gray-300" />
                  <p>No applications awaiting session creation</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {formCompleted.map((loan, index) => renderLoanCard(loan, index,
                    <Button size="sm" className="bg-violet-500 hover:bg-violet-600" onClick={() => openSessionDialog(loan)}>
                      <Calculator className="h-4 w-4 mr-1" />Create Session
                    </Button>
                  ))}
                </div>
              )}
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-green-600">{activeLoans.length + sessionCreated.length}</p>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-gray-500">
                        {loans.length > 0 ? Math.round(((activeLoans.length + sessionCreated.length) / loans.length) * 100) : 0}%
                      </span>
                    </div>
                    <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-gradient-to-r from-emerald-400 to-emerald-600 rounded-full transition-all duration-500"
                        style={{ width: `${loans.length > 0 ? Math.round(((activeLoans.length + sessionCreated.length) / loans.length) * 100) : 0}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <Card className="border-violet-200 bg-violet-50">
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <h4 className="font-semibold text-violet-800">{formCompleted.length} Sessions to Create</h4>
                          <p className="text-sm text-violet-600">Create loan session for customer approval</p>
                        </div>
  ...
  return (
    <DashboardLayout
      ...
      subtitle="Manage loan approvals and create sessions"
      menuItems={menuItems}
      activeTab={activeTab}
    ...
    >
      ...
      <Dialog open={showSessionDialog} onOpenChange={setShowSessionDialog}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><Calculator className="h-5 w-5" />Create Loan Session</DialogTitle>
            <DialogDescription>{selectedLoan?.applicationNo} - {selectedLoan?.customer?.name}</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button className="bg-emerald-500 hover:bg-emerald-600" onClick={handleCreateSession} disabled={saving}>
...
// EMI Calculator Section Component
function EMICalculatorSection() {
  ...
  const emiResult = useMemo(() => {
    if (calculatorInput.principal > 0 && calculatorInput.interestRate > 0 && calculatorInput.tenure > 0) {
      return calculateEMI(
        calculatorInput.principal,
        calculatorInput.interestRate,
        calculatorInput.tenure
      );
    }
    return null;
  }, [calculatorInput]);
    ...
    <div className="space-y-6">
      <Card className="border-0 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Calculate Equated Monthly Installment for any loan amount</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-3 gap-6">
            <div>
              ...
              <Input
                type="number"
                value={calculatorInput.principal}
                onChange={(e) => setCalculatorInput({ ...calculatorInput, principal: parseFloat(e.target.value) || 0 })}
                className="text-lg"
              />
              <input
                type="range"
                ...
                step="10000"
                value={calculatorInput.principal}
                onChange={(e) => setCalculatorInput({ ...calculatorInput, principal: parseInt(e.target.value) })}
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setCalculatorInput({ ...calculatorInput, interestRate: parseFloat(e.target.value) || 0 })}
                className="text-lg"
              />
              <input
                type="range"
                ...
                step="0.5"
                value={calculatorInput.interestRate}
                onChange={(e) => setCalculatorInput({ ...calculatorInput, interestRate: parseFloat(e.target.value) })}
                className="w-full mt-2 accent-cyan-500"
              />
              <div className="flex justify-between text-xs text-gray-500">
                <span>1%</span>
            ...
            <div>
              <Label className="flex items-center gap-2 mb-2">
                <Calendar className="h-4 w-4" /> Tenure (months)
              </Label>
              <Input
                type="number"
                value={calculatorInput.tenure}
                onChange={(e) => setCalculatorInput({ ...calculatorInput, tenure: parseInt(e.target.value) || 0 })}
                className="text-lg"
              />
              <input
                type="range"
                ...
                step="1"
                value={calculatorInput.tenure}
                onChange={(e) => setCalculatorInput({ ...calculatorInput, tenure: parseInt(e.target.value) })}
                className="w-full mt-2 accent-cyan-500"
              />
              <div className="flex justify-between text-xs text-gray-500">
                <span>3 mo</span>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid md:grid-cols-4 gap-6">
              <div className="text-center p-4 bg-white rounded-xl shadow-sm">
                ...
                <p className="text-3xl font-bold text-cyan-600">{formatCurrency(emiResult.emi)}</p>
              </div>
              <div className="text-center p-4 bg-white rounded-xl shadow-sm">
                <p className="text-sm text-gray-500 mb-1">Principal Amount</p>
                <p className="text-2xl font-bold text-gray-900">{formatCurrency(calculatorInput.principal)}</p>
              </div>
              <div className="text-center p-4 bg-white rounded-xl shadow-sm">
                <p className="text-sm text-gray-500 mb-1">Total Interest</p>
                <p className="text-2xl font-bold text-orange-600">{formatCurrency(emiResult.totalInterest)}</p>
              </div>
              <div className="text-center p-4 bg-white rounded-xl shadow-sm">
                <p className="text-sm text-gray-500 mb-1">Total Amount</p>
                <p className="text-2xl font-bold text-green-600">{formatCurrency(emiResult.totalAmount)}</p>
              </div>
            </div>

            {/* EMI Breakdown Pie Chart Representation */}
              ...
              <div className="p-4 bg-white rounded-xl shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="ml-auto font-semibold">{((calculatorInput.principal / emiResult.totalAmount) * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 bg-orange-500 rounded" />
                    <span className="text-sm text-gray-600">Interest</span>
                    <span className="ml-auto font-semibold">{((emiResult.totalInterest / emiResult.totalAmount) * 100).toFixed(1)}%</span>
                  </div>
                </div>
                <div className="mt-4 h-4 bg-gray-100 rounded-full overflow-hidden flex">
                  <div 
                    className="h-full bg-cyan-500" 
                    style={{ width: `${(calculatorInput.principal / emiResult.totalAmount) * 100}%` }} 
                  />
                  <div 
              ...
              <div className="p-4 bg-white rounded-xl shadow-sm">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{(calculatorInput.interestRate / 12).toFixed(3)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Total Payments</span>
                    <span className="font-medium">{calculatorInput.tenure}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Interest to Principal Ratio</span>
                    <span className="font-medium">{(emiResult.totalInterest / calculatorInput.principal * 100).toFixed(1)}%</span>
                  </div>
                </div>
              </div>
            </div>
