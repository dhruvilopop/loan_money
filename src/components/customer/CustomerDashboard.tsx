...
export default function CustomerDashboard() {
  ...
  const handleApproveSession = async (loan: Loan) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Track location for session confirmation
  ...
  // Filter loans
  const pendingSessionLoans = loans.filter(l => l.status === 'SESSION_CREATED');
      ...
      case 'home':
        return (
          <div className="space-y-6">
            ...
            {/* Action Required - Pending Sessions */}
            {pendingSessionLoans.length > 0 && (
              <Card className="border-amber-200 bg-amber-50 shadow-sm">
                ...
                <CardContent>
                  <p className="text-amber-700 mb-4 text-sm">You have {pendingSessionLoans.length} loan session(s) awaiting your approval.</p>
                  {pendingSessionLoans.map((loan) => (
            ...
            <Card className="border-0 shadow-sm">
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold">{settings.companyName || 'SMFC Finance'}</p>
