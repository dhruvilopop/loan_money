...
export default function OfflineLoansList({ userId, userRole, onLoanSelect }: OfflineLoansListProps) {
  ...
  // Detail dialog
  const [selectedLoan, setSelectedLoan] = useState<OfflineLoan | null>(null);
  const [detailOpen, setDetailOpen] = useState(false);
  const [loanEmis, setLoanEmis] = useState<any[]>([]);
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right ml-4">
                          ...
                          <div className="flex gap-2 mt-2">
                            <Button size="sm" variant="outline" onClick={() => handleViewLoan(loan)}><Eye className="h-4 w-4" /></Button>
          ...
          {selectedLoan && (
            <div className="space-y-6 py-4">
              ...
              {selectedLoan.company && (
                <div className="p-3 bg-blue-50 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-blue-800">{selectedLoan.company.name}</span>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-sm text-gray-500">Customer Name</p><p className="font-medium">{selectedLoan.customerName}</p></div>
                <div><p className="text-sm text-gray-500">Phone</p><p className="font-medium">{selectedLoan.customerPhone}</p></div>
                <div><p className="text-sm text-gray-500">Loan Amount</p><p className="font-bold text-lg">{formatCurrency(selectedLoan.loanAmount)}</p></div>
                <div><p className="text-sm text-gray-500">Interest Rate</p><p className="font-medium">{selectedLoan.interestRate}% p.a.</p></div>
                <div><p className="text-sm text-gray-500">EMI Amount</p><p className="font-medium text-emerald-600">{formatCurrency(selectedLoan.emiAmount)}</p></div>
                <div><p className="text-sm text-gray-500">Tenure</p><p className="font-medium">{selectedLoan.tenure} months</p></div>
              </div>

              {/* EMI Schedule */}
              <div>
                <h4 className="font-semibold mb-3">EMI Schedule</h4>
                <div className="max-h-60 overflow-y-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {loanEmis.map((emi, i) => (
                        <tr key={emi.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          <td className="p-2">{emi.installmentNumber}</td>
                          <td className="p-2">{formatDate(emi.dueDate)}</td>
                          <td className="p-2 text-right">{formatCurrency(emi.totalAmount)}</td>
                          <td className="p-2 text-center">
                            <Badge variant={emi.paymentStatus === 'PAID' ? 'default' : emi.paymentStatus === 'OVERDUE' ? 'destructive' : 'outline'}>
                              {emi.paymentStatus}
                            </Badge>
                          </td>
                        </tr>
