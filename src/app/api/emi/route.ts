...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { emiId, action, data, loanId, paidAmount, paymentMode, paymentRef, creditType, remarks, proofUrl, userId } = body;
    ...
    if (action === 'pay' || paidAmount !== undefined) {
      ...
      const result = await db.$transaction(async (tx) => {
        ...
        const updatedEmi = await tx.eMISchedule.update({
          where: { id: emiId },
          data: {
            paymentStatus: paidAmount >= emi.totalAmount ? EMIPaymentStatus.PAID : EMIPaymentStatus.PARTIALLY_PAID,
