...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (action === 'pay-emi' && emiId && userId) {
      const { paymentMode, paymentReference, amount, paymentType, bankAccountId } = body;
      ...
      const user = await db.user.findUnique({
        where: { id: userId },
        select: { id: true, credit: true, role: true, name: true }
      });
      ...
      const newCredit = user.credit + paymentAmount;
      ...
      if (paymentMode === 'CASH' && targetBankAccountId && emi.offlineLoan.companyId) {
        try {
          bankTransactionResult = await processBankTransaction({
            ...
            description: `EMI #${emi.installmentNumber} collected for ${emi.offlineLoan.loanNumber} - ${paymentType}`,
